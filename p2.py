    ...
    if uploaded_file is not None:
        try:
            ...
            if 'Condition' in df.columns and not df['Condition'].isna().all():
                st.markdown("#### Book Condition Analysis")
                fig4, ax4 = plt.subplots(figsize=(10, 6))
                condition_counts = df['Condition'].value_counts()
                colors = ['#2ecc71' if x == 'Good' else '#e74c3c' if x in ['Damaged', 'Lost'] else '#f39c12' for x in condition_counts.index]
                bars = ax4.bar(condition_counts.index, condition_counts.values, color=colors)
                ax4.set_xlabel('Condition')
                ax4.set_ylabel('Number of Books')
                ax4.set_title('Book Condition Distribution')
                plt.xticks(rotation=45)
                ...
                if 'Status' in df.columns or 'Condition' in df.columns:
        
                 if 'Status' in df.columns:
                     a=target_conditions = ['Lost', 'Damaged', 'To Be Replaced','Good']
        
        
                     condition_counts = a['Condition'].str.strip().value_counts()
                     issued_condition_counts = condition_counts.reindex(target_conditions, fill_value=0)
        
       
                     st.subheader("📦 Issued Books Condition Breakdown")
                     fig, ax = plt.subplots()
                     issued_condition_counts.plot(
                       kind="pie",
                       autopct="%1.1f%%",
                       startangle=140,
                       ax=ax,
                       colors=['#ff6b6b', '#ffa94d', '#74c0fc', '#51cf66']
              )
            ax.set_ylabel("")  # remove y-label
            ax.set_title("Lost / Damaged / To Be Replaced/ Good (Issued Books)")
            st.pyplot(fig)
